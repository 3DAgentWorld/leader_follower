...
# -*- coding: utf-8 -*-
"""
Convert ONUW game logs to verl GRPO training data format.

Usage:
    python scripts/convert_logs_to_grpo_data.py --log_dir logs/onuw/battle --output output.jsonl
...
from prompt.onuw_prompts import (
    system_prompt as onuw_system_prompt_template,
    refine_prompt as onuw_refine_prompt,
    measurer_system_prompt as onuw_measurer_system_prompt,
    measurer_user_prompt as onuw_measurer_user_prompt,
    role_introduction,
    role_target,
    init_strategies
)
...
def build_system_prompt(name: str, role: str, strategy: str = None,
    ...
    return onuw_system_prompt_template.format(
        name=name,
        role=role,
        strategy=strategy,
...
def parse_process_json(process_file: str) -> Dict[str, List[Dict]]:
    ...
    Returns:
        Parsed game log data, organized by phases (night, day, voting).
    """
...
def extract_player_mapping(process_data: Dict) -> Dict[str, str]:
    ...
    """
    # First try game_result section
    if "game_result" in process_data:
        result = process_data["game_result"]
        if "initial_roles" in result:
            return result["initial_roles"]
    
    # Fallback: scan events
    player_mapping = {}
    for phase_key, events in process_data.items():
        if phase_key == "game_result":
            continue
...
def build_dialogue_summary(process_data: Dict, current_phase: str,
                           current_event_idx: int) -> str:
    """
    Build a dialogue history summary (text format) up to the current event.
    
    In ONUW, phases are: night, day, voting (not round_X).
    
    Args:
        process_data: Full game log data.
        current_phase: Current phase key (e.g., "day").
        current_event_idx: Index of the current event in the phase.
    ...
    summary_lines = []
    # Define phase order
    phase_order = ["night", "day", "voting"]
    
    phases = [p for p in phase_order if p in process_data]
    
    try:
        current_phase_idx = phases.index(current_phase)
    except ValueError:
        current_phase_idx = len(phases) - 1
    
    # Iterate over all previous phases
    for i in range(current_phase_idx + 1):
        phase_key = phases[i]
        events = process_data[phase_key]
        
        if not isinstance(events, list):
            continue
        
        # If current phase, only process up to the current event
        max_idx = current_event_idx if phase_key == current_phase else len(events)
        
        for j in range(max_idx):
            event = events[j]
            
            # Extract Host messages (public ones only)
            if "Host" in event:
...
def build_refiner_prompt(
    ...
    """
    refine_input = onuw_refine_prompt.format(
...
def build_measurer_prompt_template(
    ...
    summary = build_dialogue_summary(
        process_data, phase_key, current_event_idx
    )
    ...
    system_content = onuw_measurer_system_prompt.format(
        name=follower_name,
        role=follower_role,
    ...
    user_content = onuw_measurer_user_prompt.format(
        name=follower_name,
        phase=f"discussion, Phase: {phase_key}",
        role=follower_role,
...
def convert_discuss_event_to_training_sample(
    ...
    dialog_history = build_dialogue_summary(
        process_data, phase_key, event_idx
    )
    ...
    # Build game state
    game_state = f"Phase: discussion, {phase_key}"
    ...
    phase_events = process_data[phase_key]
    ...
    for i in range(event_idx + 1, len(phase_events)):
        next_event = phase_events[i]
    ...
    next_event = phase_events[next_event_idx]
...
def convert_game_logs_to_grpo_data(
    ...
    Args:
        ...
        only_discuss: Whether to only process discussion phase (day) data.
    ...
    for game_dir in game_dirs:
        ...
        # In ONUW, phases are: night, day, voting
        # Discussion events are in the "day" phase
        for phase_key, events in process_data.items():
            if phase_key == "game_result":
                continue
            if not isinstance(events, list):
            ...
            for event_idx, event in enumerate(events):
                ...
                is_discuss_event = "discuss" in host_msg or \
                                   ...
                                   "share your thoughts" in host_msg or \
                                   "it is your turn" in host_msg
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert ONUW game logs to verl GRPO training data format"
    )
    parser.add_argument(
        "--log_dir",
        type=str,
